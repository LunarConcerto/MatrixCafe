...
public class CommandLineNoTypeParser implements CommandParser{
    ...
    @Override
    public CommandActionParam parse(@NotNull SessionMessage message) {
        ...
        if (length == 1){
            return minArgLength == 0 ? CommandActionParam.empty(message) : CommandActionParam.dissatisfy(message);
        }
        ...
        if (argLength < minArgLength){
            return CommandActionParam.dissatisfy(message, CommandActionParam.DissatisfyReason.ARGUMENT_TOO_SHORT);
        }

        if (argLength > maxArgLength){
            return CommandActionParam.dissatisfy(message, CommandActionParam.DissatisfyReason.ARGUMENT_TOO_LONG);
        }

        CommandActionParam data = createActionData(message, argLength, optLength);
    ...
    @Contract(pure = true)
    private CommandActionParam parse(String @NotNull [] optionList, CommandActionParam actionData){
            ...
            switch (optionType(optionText)){
                case ARGUMENT -> {
                    if (i+1 < optionList.length && !isOption(optionText)){
                        ...
                        return CommandActionParam.dissatisfy(actionData.message(), CommandActionParam.DissatisfyReason.PATTERN_ERROR);
                    }
                }
    ...
    private CommandActionParam createActionData(SessionMessage message, int argLength, int optLength){
        if (argLength > 0 && optLength > 0){
            return CommandActionParam.withArgumentAndOptionData(message);
        }

        if (argLength > 0 && optLength == 0){
            return CommandActionParam.withArgumentData(message);
        }

        if (argLength == 0 && optLength > 0){
            return CommandActionParam.withOptionData(message);
        }

        return CommandActionParam.dissatisfy(message);
    }
